	...
	public byte[] gene_mess(int mess_type, int parameter) throws IOException {
		...
		case 6: {
			...
			if(!Miss_Piece.isEmpty()) {
				...
				System.out.println("actual piecenum" + host_peer.dl_num);
				return new byte[0];
			}
		}
		}
...
//		System.out.println("mess type " + type);
		...
		case 1: {
			++requestlost;
			if(requestlost > 2 * neighbours.size()) {
				for(int i : Req_Piece) {
					turn_bits(Request, i, 0);
				}
				for(int i = 0; i != Req_Piece.size(); ++i) {
					Req_Piece.set(i, -1);
				}
				System.out.println("Request lost");
				for(int i = 0; i != Request.size(); ++i) {
					Request.set(i, host_peer.piece.get(i));
				}
			}
...
//			System.out.println("unfinish peers after bitfield:" + terminate);
		...
		//	System.out.println("receive piece is" + index);
			...
			Boolean rece = file.get(index).length == 0;
			
			
			if(rece) file.set(index, payload2);
